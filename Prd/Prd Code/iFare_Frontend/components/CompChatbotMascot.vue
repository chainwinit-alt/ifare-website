...
const seatedBubbleMessages = [
  '智慧小幫手目前暫停開放，請先使用頁面功能查詢。',
  '你可以從 i-Fare、最新消息與公益夥伴找到需要的資訊。',
] as const;
...
const patrolStyle = computed(() => ({
  transform: `translateX(${props.isOpen ? 24 : prefersReducedMotion.value ? 0 : patrolDirection.value > 0 ? (isCompactViewport.value ? 34 : 48) : 0}px)`,
}));
...
const leftArmStyle = computed(() => ({
  transform: isDragging.value
    ? `rotate(${clamp(58 + Math.sin(limbSwingPhase.value) * 7 + dragSwingX.value * 0.06, 48, 70)}deg)`
    : undefined,
}));

const rightArmStyle = computed(() => ({
  transform: isDragging.value
    ? `rotate(${clamp(-58 - Math.sin(limbSwingPhase.value) * 7 + dragSwingX.value * 0.06, -70, -48)}deg)`
    : undefined,
}));
...
.mascot-bubble {
  ...
  right: 48px;
  top: -66px;
  width: min(150px, calc(100vw - 64px));
