  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Code_Recipient_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type { ColumnInfo, TbDataInfo_CodeRecipient } from "@/interface/MTable";
...
const tbDataList = reactive<Array<TbDataInfo_CodeRecipient>>([]);
...
function WebAPI_GetDataList(
  ...
  $WebAPI.GetCodeRecipient(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_CodeRecipient> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
