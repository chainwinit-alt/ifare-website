  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Articles_Welfare_Add')"
...
import type {
  ColumnInfo,
  TbDataInfo_ArticlesWelfare,
} from "@/interface/MTable";
import dataTest from "@/data/TestDataList/ArticlesWelfare.json";
...
const tbDataList = reactive<Array<TbDataInfo_ArticlesWelfare>>([]);
...
function WebAPI_GetDataList(
  ...
  $WebAPI.GetArticlesWelfareList(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_ArticlesWelfare> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
            policy: item.codePolicy_LabelName,
            title: item.title,
            keyword: item.codeKeywordList.map((item:any, i:number) => {
