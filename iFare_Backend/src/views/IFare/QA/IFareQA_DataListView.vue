  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('IFare_QA_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type { ColumnInfo, TbDataInfo_IFareQA } from "@/interface/MTable";
import dataTest from "@/data/TestDataList/IFareQA.json";
...
const tbDataList = reactive<Array<TbDataInfo_IFareQA>>([]);
...
function WebAPI_GetDataList(
  ...
  $WebAPI.GetFareQAList(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_IFareQA> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
            title: item.question,
