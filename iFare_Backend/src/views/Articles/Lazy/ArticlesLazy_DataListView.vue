  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Articles_Lazy_Add')"
...
import type { ColumnInfo, TbDataInfo_ArticlesLazy } from "@/interface/MTable";
import dataTest from "@/data/TestDataList/ArticlesLazy.json";
...
const tbDataList = reactive<Array<TbDataInfo_ArticlesLazy>>([]);
...
function WebAPI_GetDataList(
  ...
  $WebAPI.GetArticlesLazyList(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_ArticlesLazy> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
            title: item.title,
