  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Code_Identity_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type { ColumnInfo, TbDataInfo_CodeIdentity } from "@/interface/MTable";
import dataTest from "@/data/TestDataList/Code_Identity.json";
...
const columnInfoList = reactive<Array<ColumnInfo>>([
  ...
  { prop: "date_update", label: "異動日期" },
  {
    prop: "operate",
    label: "操作",
    opts: { type: "url", info: [{ label: "編輯" }] },
  },
]);
const tbDataList = reactive<Array<TbDataInfo_CodeIdentity>>([]);
  ...
  $WebAPI.GetCodeIdentity(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_CodeIdentity> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
