  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Code_Policy_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type { ColumnInfo, TbDataInfo_CodePolicy } from "@/interface/MTable";
import dataTest from "@/data/TestDataList/Code_Policy.json";
...
const tbDataList = reactive<Array<TbDataInfo_CodePolicy>>([]);
  ...
  $WebAPI.GetCodePolicy(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_CodePolicy> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
