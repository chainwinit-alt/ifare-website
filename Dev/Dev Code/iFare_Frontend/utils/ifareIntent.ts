 ...
 *
 * 唯一的例外是「发」：它同時對應「發」與「髮」，一律轉「發」會把「假发」「理发」
 * 轉成站內永遠對不上的「假發」「理發」（語料寫的是「假髮」「理髮」）。所以先用
 * 詞組把「髮」的那一義挑出來，剩下的才走「發」——順序不能顛倒。
 */
const SIMPLIFIED_TO_TRADITIONAL: Array<[RegExp, string]> = [
  // 「发」的「髮」義：必須排在通則 发→發 之前
  [/假发/gu, "假髮"], [/理发/gu, "理髮"], [/头发|頭发/gu, "頭髮"],
  [/医/gu, "醫"], [/疗/gu, "療"], [/补/gu, "補"], [/贴/gu, "貼"], [/户/gu, "戶"],
  // 「经济弱势」是站內經濟條件的正式名稱，少了 经／势 兩個字就轉不完整、照樣查 0 筆
  [/经/gu, "經"], [/势/gu, "勢"],
 ...
 *
 * 「沒」後面的「有」在口語裡常被省略——「我沒身障手冊還能申請嗎」原本因為只收
 * 「沒有／沒領」而整句漏判，資格問題被答反。這裡用 沒(?:有|領)? 一併涵蓋；因為視窗
 * 切到條件詞為止，`沒$` 就代表「沒」緊貼著條件詞（沒低收、沒身心障礙），不會誤中
 * 「沒問題」「出沒」這種後面接別的字的用法。「不具備」同理：原本 `不具$` 對不上。
 */
...
const SITUATION_VOCABULARY: Array<[RegExp, string]> = [
  ...
  [/長照|長期照顧/u, "長照 長期照顧"],
  [/早療|早期療育/u, "早療 早期療育"],
  // 「中風」要擋掉「台中風災」「台中風景」——那兩個字只是縣市名跟下一個詞剛好夾出來的，
  // 一律命中會把台中的災害查詢整批導去長照。用「前一個字不是台／臺」判斷，
  // 句首的「中風」（使用者只打兩個字）仍然算。
  [/腦中風|(?:^|[^台臺])中風|癱瘓|半身不遂|臥床|插鼻胃管|無法自理|不能自理|生活不能自理|沒辦法自己(?:吃飯|洗澡|穿衣|上廁所|走路)|需要人(?:照顧|看護)|行動不便/u, "失能 長期照顧 無法自理"],
  [/失智|阿茲海默|老年痴呆|記憶力(?:變差|退化|越來越差|不好)|忘東忘西|認知退化/u, "失智 長期照顧"],
  // 「照顧＋親屬」是長照家屬最直接的求助句，句子裡卻沒有半個政策詞——
  // 實測「我要照顧我爸爸」「我要照顧我爺爺」直查都是 0 筆，站內「長期照顧」有 172 筆。
  // 中間容許 3 個字的插入語（「照顧生病的媽媽」）但不跨標點；插入語裡也不准出現
  // 小／孩／子：「照顧小孩的爸媽」講的是托育，不該被拉去長照。
  [/(?:照顧|顧)[^\s，,。；;、小孩子]{0,3}(?:爸|媽|爺爺|奶奶|阿公|阿嬤|外公|外婆|公婆|公公|婆婆|岳父|岳母|長輩|老人家|父母|父親|母親)/u, "長照 長期照顧"],
  // 照顧的人自己撐不住的講法。展開詞挑「喘息」不挑「喘息服務」：「喘息」31 筆都準，
  // 「喘息服務」被「服務」這個泛字撐到 179 筆。「想喘口氣」是喘息服務文宣的固定句，一併收。
  [/(?:照顧|顧)(?:得|到)?[^\s，,。；;、]{0,3}(?:好累|很累|太累|累壞|累垮|喘不過氣)|照顧(?:壓力|疲憊|疲勞)|快被照顧(?:壓垮|拖垮)|(?:想|需要)喘口氣/u, "喘息 長期照顧"],
  // 家屬找機構時講「養老院」「安養院」，站內寫「住宿式」「長照機構」——
  // 實測「養老院」只有 11 筆，「住宿式照顧」181 筆、「長照機構」83 筆。
  [/養老院|安養院|安養中心|護理之家|住機構|送機構/u, "住宿式照顧 長照機構"],
  // 「日照（中心）」是日間照顧的通行簡稱，語料卻一律寫全稱——實測「日照中心」0 筆、
  // 「日間照顧」170 筆。(?!顧)、(?!育|嬰) 是在排除「假日照顧」「平日托育」這種
  // 剛好夾出「日照」「日托」二字的句子，那些講的不是日間照顧。
  [/日照(?!顧)|日托(?!育|嬰)/u, "日間照顧"],
  ...
  // 民眾講的是器材名，單一器材在站內只有個位數（助聽器 2 筆、拐杖 2 筆、氣墊床 2 筆、
  // 輪椅 18 筆），補上「輔具」（63 筆）才看得到租借與補助的全貌。
  [/輪椅|助行器|助聽器|拐杖|氣墊床|爬梯機/u, "輔具"],
  // 房子壞了的講法在語料裡不存在——實測「漏水」0 筆，而「修繕」29 筆、「住宅」99 筆。
  // 只收「明確在講房屋損壞」的組合，「壞掉」單獨出現什麼都可能壞，不能收。
  [/漏水|屋頂(?:破|壞|塌|漏)|房子(?:很)?(?:老舊|破舊|壞掉)|(?:修|整修)(?:房子|屋頂|浴室|廁所)/u, "修繕 住宅"],
  ...
  // 「小孩沒人帶」是托育需求最常見的講法，實測直查 0 筆；站內寫的是托育（68 筆）、
  // 托嬰（38 筆）。「保姆」是「保母」的常見寫法，語料只有「保母」——「保姆」直查 0 筆。
  [/(?:小孩|孩子|寶寶|嬰兒|幼兒|小朋友)[^\s，,。；;、]{0,3}沒(?:人|地方)(?:帶|顧|照顧|托)|沒人(?:帶|顧)(?:小孩|孩子|寶寶|嬰兒|幼兒|小朋友)|保母|保姆|托嬰/u, "托育 托嬰"],
  // 單親家長的自我描述是「我一個人帶小孩」，沒有人會自稱「特殊境遇」——
  // 實測直查 0 筆，「特殊境遇」118 筆、「單親」29 筆。限定「一個人／獨自／獨力」開頭：
  // 「自己帶小孩」刻意不收，在家帶小孩請育嬰假的人也這樣講，收了會被誤導向特境。
  [/(?:一個人|獨自|獨力)(?:帶|養|扶養|撫養)[^\s，,。；;、]{0,3}(?:小孩|孩子|女兒|兒子)|單親/u, "特殊境遇 單親"],
  // 受暴的人打的是「被老公打」「保護令」，不是政策分類名。「家暴」直查只有 14 筆，
  // 補上保護令（45 筆）與特殊境遇（118 筆）才涵蓋庇護與扶助那一片。
  // 「打」限定在加害人稱謂後至多 2 個字，且排除「打電話」「打招呼」這類慣用語——
  // 「被家人打電話罵」是在講通訊，不是受暴。「打呼」「打點滴」同理（「被老公打呼吵醒」
  // 「被家人打點滴時…」講的都不是暴力），一併排除。
  [/家暴|被(?:老公|先生|丈夫|老婆|太太|配偶|男友|女友|家人)[^\s，,。；;、]{0,2}(?:打(?!電話|招呼|呼|工|疫苗|針|氣|點滴)|施暴|動手)|(?:遭受?|受到)(?:家庭)?暴力|保護令/u, "家暴 保護令 特殊境遇"],
  ...
  // 「社宅」是社會住宅的通行簡稱、「包租代管」是它的方案名，語料卻只寫全稱——
  // 實測兩個詞直查都是 0 筆，「社會住宅」145 筆。
  [/社宅|包租代管/u, "社會住宅 租屋"],
  ...
  // 「學貸」「就學貸款」同一回事：兩個詞直查都是 0 筆，也只放左邊。
  [/學費|學雜費|註冊費|補習費|書籍費|學貸|就學貸款|念不起(?:書|大學)|讀不起(?:書|大學)|上不起學/u, "就學 教育 助學"],
  ...
  // 「家裡突然出事」是急難救助的典型入口，但「出事」「變故」在語料裡不存在——
  // 實測「家裡突然出事需要幫忙」直查 0 筆，急難 65 筆、生活扶助 254 筆。
  // 「車禍」也掛在這裡：直查 0 筆，站內最接近的入口就是急難救助。
  // 「出事／出狀況」一定要跟著「家裡／家人」，不然「工作出狀況」也會被拉進來。
  [/(?:家裡|家中|家人)[^\s，,。；;、]{0,3}(?:出事|出狀況|發生(?:意外|變故|事故))|突然(?:出事|發生(?:意外|變故|事故))|遭逢(?:變故|意外|巨變)|車禍|急需(?:幫忙|幫助|援助|救助|用錢)/u, "急難 生活扶助"],
  // 天災的講法對不上政策詞「災害（救助）」——「火災」直查有 28 筆，但「失火」
  // 「災損」是 0 筆，「地震」只有 4 筆；災害 38 筆、急難 65 筆這兩個詞才是全貌。
  [/火災|失火|燒毀|淹水|水災|風災|震災|受災|災損|地震/u, "災害 急難"],
  ...
  // 「罕病」是罕見疾病的通行簡稱，但兩字在語料裡不相連——實測「罕病」0 筆、
  // 「罕見疾病」62 筆。「健檢」同病：0 筆對 94 筆。
  [/罕病/u, "罕見疾病"],
  [/健檢/u, "健康檢查"],
  // 心情低落的人不會用「心理健康」描述自己——實測「心情很低落想找人談」直查 0 筆，
  // 「憂鬱」「失眠」「想不開」也全是 0 筆，站內寫的是「心理健康」（144 筆）。
  // 「諮商」直查 0 筆，放進展開詞等於沒修，所以右邊只有「心理健康」一個詞。
  [/心情(?:很|好|非常|一直)?(?:低落|不好|差|鬱悶|沮喪)|憂鬱|焦慮|想不開|情緒(?:困擾|低落|不穩|失控)|壓力(?:很|好|太)大|睡不著|失眠|心理(?:諮商|輔導|支持|壓力)|找人(?:談|聊)/u, "心理健康"],
  // 法律糾紛的口語對不上「法律扶助／協助」——實測「打官司」「官司」直查 0 筆，
  // 法律扶助 86 筆、法律協助 50 筆。「被告」不能單收（「被告知」會誤中），
  // 要求「告」後面接「了」或「上法院」。
  [/打官司|吃官司|官司|訴訟|請不起律師|法律(?:糾紛|問題|諮詢)|被(?:人|房東|雇主)?告(?:了|上法院)/u, "法律扶助 法律協助"],
  [/獨居|沒人照顧|一個人住/u, "獨居"],
  // 「平安鈴」「救命鈴」是緊急救援系統在各縣市的通行叫法，語料只寫「緊急救援」——
  // 實測「平安鈴」0 筆、「緊急救援」47 筆。
  [/平安鈴|救命鈴|緊急按鈕/u, "緊急救援"],
  [/往生|過世|辦後事|喪事/u, "喪葬"],
  // 環保葬的問法在語料裡完全不存在——連官方關鍵字「環保自然葬」「聯合奠祭」本身
  // 直查都是 0 筆（沒有政策文字含這兩個詞）。只能先導到「喪葬」（27 筆）這個
  // 上位入口；在語料補上環保葬字樣之前，這是唯一查得到的路。
  [/樹葬|海葬|花葬|灑葬|環保葬|自然葬|公祭|奠祭/u, "喪葬"],
  // 想投入服務的人打「志工」，語料寫「志願服務」——實測「志工」只有 1 筆、
  // 「志願服務」178 筆。
  [/志工/u, "志願服務"],
];
...
//
// 「子女」單獨出現不算：老人福利政策的資格欄位很常寫「未受子女扶養」「子女無扶養
// 能力」，那是在描述長輩的處境、受助者就是長輩本人，只看「子女」二字會把這些政策
// 誤判成「給孩子的」而被降權。所以「子女」必須跟就學／助學／學費／教育連用
//（中間允許 6 個字的插入語，但不跨標點，避免「未受子女扶養，且就學…」跨句誤中）。
const POLICY_CHILD_BENEFICIARY_PATTERN =
  /就學|助學|學費|學雜費|子女[^。，,、；;]{0,6}教育/u;
