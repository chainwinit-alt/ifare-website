  ...
  <section class="ifare-summary-card" :data-provider="selectedProvider">
    <div class="summary-head">
      <div class="summary-head-copy">
        ...
        <h3 class="summary-title">案例重點說明</h3>
      </div>
      ...
      <div class="summary-tools">
        ...
        <button class="summary-retry" type="button" @click="loadSummary(true)" :disabled="isLoading">
          {{ isLoading ? "生成中" : "重新摘要" }}
        </button> -->
      </div>
    </div>
    <div class="summary-body">
      <div v-if="isLoading && !summaryText.trim()" class="summary-loading">
        ...
        <p class="summary-loading-text">AI 正在整理摘要</p>
      </div>
      ...
      <div v-if="isLoading" class="summary-cursor">▍</div>
    ...
    <div class="summary-foot">
      <span class="summary-meta">{{ cases.length }} 筆資料</span>
      <span class="summary-meta" v-if="isLoading">生成中</span>
    </div>
  </section>
</template>
...
function rankCases(query: string, cases: SummaryCaseItem[]): RankedSummaryCaseItem[] {
  ...
  const ranked = cases
    .map((item) => {
      ...
      const combined = `${title}${area}${qualification}`;
      const fieldWeights = [
        [title, 16],
        [area, 5],
        [qualification, 10],
...
const fallbackText = computed(() => {
  if (!props.cases.length) {
    return "- 先完成搜尋，AI 會自動整理摘要。";
  }

  return "- AI 正在整理這次搜尋結果。";
});

const summaryDisplayText = computed(() => {
  if (summaryText.value.trim()) return summaryText.value.trim();
  if (!props.cases.length && hasSearchContext.value) return "";
...
const referenceCases = computed<ReferencedCaseItem[]>(() => {
  const referenceNumbers = parseReferenceNumbers(summaryText.value);
  if (!referenceNumbers.length) return [];

  return referenceNumbers
    .map((referenceNo, index) => {
      const item = rankedCases.value[referenceNo - 1];
      if (!item) return null;
      return {
        ...item,
        referenceNo: index + 1,
    ...
    .filter((item): item is ReferencedCaseItem => Boolean(item));
});

const actualReferenceCases = referenceCases;

function parseReferenceNumbers(text: string) {
  const normalizedText = normalizeReferenceNotation(text || "");
  referenceTokenPattern.lastIndex = 0;
  const matches = Array.from(normalizedText.matchAll(referenceTokenPattern));
  return Array.from(
    new Set(
      matches
...
async function loadSummary(forceRefresh = false) {
  ...
  try {
    ...
    streamError.value = "摘要串流暫時失敗，已切換到本地顯示。";
