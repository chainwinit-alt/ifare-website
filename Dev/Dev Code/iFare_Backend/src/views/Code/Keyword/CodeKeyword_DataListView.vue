  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Code_Keyword_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type { ColumnInfo, TbDataInfo_CodeKeyword } from "@/interface/MTable";
import dataTest from "@/data/TestDataList/Code_Keyword.json";
...
const tbDataList = reactive<Array<TbDataInfo_CodeKeyword>>([]);
  ...
  $WebAPI.GetCodeKeyword(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_CodeKeyword> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
