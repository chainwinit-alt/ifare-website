<template>
  <main-header>
    <template #subtitle v-if="$route.name != 'Code_Keyword_Add'">
  ...
  <el-scrollbar class="main-scrollbar">
    <div
      class="section-main-card card-fullsize card-code-keyword card-input-format"
    >
      <div class="card-info">
...
async function SaveAction() {
  ...
  if (routeNameType.indexOf("add") >= 0) {
    ...
    const dupAdd = await checkDuplicate($WebAPI.GetCodeKeyword, _labelName, null);
    if (!dupAdd.ok) {
      $Message({
    ...
    $WebAPI.InsertCodeKeyword(
      ...
      (res: any) => {
        ...
        $commonLib.GuideToPage("Code_Keyword_DataList");
      }
    );
  }
  ...
  if (routeNameType.indexOf("edit") >= 0) {
    ...
    const dupEdit = await checkDuplicate($WebAPI.GetCodeKeyword, _labelName, _id);
    if (!dupEdit.ok) {
      $Message({
