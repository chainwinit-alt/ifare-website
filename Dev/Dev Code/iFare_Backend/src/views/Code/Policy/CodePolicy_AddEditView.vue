<template>
  <main-header>
    <template #subtitle v-if="$route.name != 'Code_Policy_Add'">
  ...
  <el-scrollbar class="main-scrollbar">
    <div
      class="section-main-card card-fullsize card-code-policy card-input-format"
    >
      <div class="card-info">
...
async function SaveAction() {
  ...
  if (routeNameType.indexOf("add") >= 0) {
    ...
    $WebAPI.InsertCodePolicy(
      ...
      (res: any) => {
        ...
        $commonLib.GuideToPage("Code_Policy_DataList");
      }
    );
  }
  ...
  if (routeNameType.indexOf("edit") >= 0) {
    ...
    const dupEdit = await checkDuplicate($WebAPI.GetCodePolicy, _labelName, _id);
    if (!dupEdit.ok) {
      $Message({
