<template>
  <!-- ========================================================
    頁面用途：身分別代碼【新增 / 編輯】表單頁
    路由名稱：Code_Identity_Add（新增）、Code_Identity_Edit（編輯）
    功能說明：
      - 新增模式：填寫名稱與啟用狀態後，呼叫 InsertCodeIdentity API 建立新資料
      - 編輯模式：從 URL query.id 讀取對應資料，修改後呼叫 UpdateCodeIdentity API 更新
  ======================================================== -->

  <!-- 頂部標題列，包含副標題與操作按鈕 -->
  <main-header>
    <!-- 編輯模式下才顯示副標題（建立日期 + 資料ID） -->
    <template #subtitle v-if="$route.name != 'Code_Identity_Add'">
  ...
  <el-scrollbar class="main-scrollbar">
    <!-- 卡片區塊：名稱輸入 -->
    <div
      class="section-main-card card-fullsize card-code-identity card-input-format"
    >
      <div class="card-info">
...
// 編輯模式：頁面載入時呼叫 GetCodeIdentity API 取得既有資料填入表單
if (routeNameType.indexOf("edit") >= 0) {
  // API 說明：GetCodeIdentity(token, 建立日起, 建立日迄, 異動日起, 異動日迄, 名稱關鍵字, 是否只取啟用, ids陣列, callback)
  ...
  if (routeNameType.indexOf("add") >= 0) {
    ...
    $WebAPI.InsertCodeIdentity(
      ...
      (res: any) => {
        ...
        $commonLib.GuideToPage("Code_Identity_DataList");
      }
    );
  }
