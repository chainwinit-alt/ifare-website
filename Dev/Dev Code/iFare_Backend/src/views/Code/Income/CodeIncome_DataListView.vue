  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Code_Income_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type { ColumnInfo, TbDataInfo_CodeIncome } from "@/interface/MTable";
import dataTest from "@/data/TestDataList/Code_Income.json";
...
const tbDataList = reactive<Array<TbDataInfo_CodeIncome>>([]);
...
function WebAPI_GetDataList(
  ...
  $WebAPI.GetCodeIncome(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_CodeIncome> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
