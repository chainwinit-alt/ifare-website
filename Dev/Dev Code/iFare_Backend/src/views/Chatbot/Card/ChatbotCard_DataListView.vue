  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('Chatbot_Card_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type { ColumnInfo, TbDataInfo_ChatbotCard } from "@/interface/MTable";
...
const tbDataList = reactive<Array<TbDataInfo_ChatbotCard>>([]);
...
function WebAPI_GetDataList(
  ...
  $WebAPI.GetChatbotCardList(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_ChatbotCard> = _res.result.map((item: any) => {
        return {
          id: item.id,
          title: item.title,
...
WebAPI_GetDataList(
  _route.query.create?.toString().split("TO")[0],
