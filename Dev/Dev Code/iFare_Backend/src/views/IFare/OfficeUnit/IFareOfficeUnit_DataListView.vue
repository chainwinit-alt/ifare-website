  ...
  <main-header>
    <template #btnsRight>
      <el-button
        ...
        type="primary"
        size="large"
        @click="$commonLib.GuideToPage('IFare_OfficeUnit_Add')"
        >新增資料</el-button
      >
    </template>
  </main-header>
...
import type {
  ColumnInfo,
  TbDataInfo_IFareOfficeUnit,
} from "@/interface/MTable";
import dataTest from "@/data/TestDataList/IFareOfficeUnit.json";
...
const tbDataList = reactive<Array<TbDataInfo_IFareOfficeUnit>>([]);
...
function WebAPI_GetDataList(
  ...
  $WebAPI.GetFareOfficeUnitList(
    ...
    (res: any) => {
      ...
      let list: Array<TbDataInfo_IFareOfficeUnit> = _res.result.map(
        (item: any, i: number) => {
          return {
            id: item.id,
            title: item.title,
