...
for (let r = 2; r <= range.e.r; r += 1) {
  if (Number(ws[cell(r, 0)]?.v) !== 7) continue;
  ws[cell(r, 13)] = { ...(ws[cell(r, 13)] || {}), v: '部分修正', t: 's' };
  ws[cell(r, 14)] = { ...(ws[cell(r, 14)] || {}), v: '2026-05-11', t: 's' };
  ws[cell(r, 15)] = {
    ...(ws[cell(r, 15)] || {}),
    v: '先統一手機版結果卡片的資格文字截斷方式：改為兩行 clamp + word-break，避免文字長度不同時卡片高度與收斂方式不一致；桌機版仍可再視需求微調。',
    t: 's',
  };
  updated = true;
  break;
...
if (!updated) throw new Error('Issue #7 not found');
...
if (summary) {
  ...
  summary['G3'] = { ...(summary['G3'] || {}), v: '已補強 #7 結果頁資格文字手機版截斷一致性', t: 's' };
}
